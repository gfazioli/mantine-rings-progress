...
Each ring supports a `tooltip` prop for hover content. You can customize the tooltip with `tooltipProps` per-ring or globally on the component. The tooltip is rendered using Mantine's `Tooltip.Floating`.
